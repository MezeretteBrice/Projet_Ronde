    ...
    cheminPhoto(_cheminPhoto)
{
    ...
    ui->checkBoxFiltreDate->setChecked(true);
    ui->checkBoxFiltreNomRonde->setChecked(true);
    ui->pushButtonDate2->hide();

    //ui->tableWidgetRondes->hide();
    ui->tableWidget_Pointeaux->hide();
    ui->graphicViewPhotos->hide();
    ui->textEditCommentaire->hide();
    ui->labelCommentaire->hide();
...
void IHMRapport::AfficherListeRapport()
{
    ...
    for(int i = 0 ; i<listeRapport.size() ; i++)
    {
        ...
        ui->tableWidgetRondes->setItem(i, 3, nbPointeauTague);
